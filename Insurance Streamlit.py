...
# Load the trained insurance premium prediction model
try:
...
# Input fields for insurance features (matching the X_train_3 columns)
# Columns expected by random_search_3 model:
...
# Height input in feet and inches, then convert to cm
st.subheader('Height')
...
# Display converted height for user's reference (optional)
...
# The random_search_3 model (best_rf_model_3) was trained WITHOUT BMI, AgeBin, BMIbin
# and no explicit categorical encoding for the binary features as they were treated as numerical.
...
def model_predict(
    ...
    # Create a DataFrame with the features in the exact order expected by the model
    # The columns should match X_train_3 exactly:
    # ['Age', 'Diabetes', 'BloodPressureProblems', 'AnyTransplants', 'AnyChronicDiseases',
    # 'Height', 'Weight', 'KnownAllergies', 'HistoryOfCancerInFamily', 'NumberOfMajorSurgeries']
